...
public class Room : MonoBehaviour
{
    ...
    private void GetTiles()
    {
        ...
        foreach (Vector3Int tilePos in extraTilemap.cellBounds.allPositionsWithin)
        {
            ...
            Vector3 newPosition = 
                new Vector3(position.x + 0.5f, position.y + 0.5f, position.z);
    ...
    private void GenerateRoomUsingTemplate()
    {
        ...
        int randomIndex =
            Random.Range(0, LevelManager.Instance.RoomTemplates.Templates.Length);
            ...
            for (int x = 0; x < texture.width; x++, a++)
            {
                ...
                foreach (RoomProp prop in LevelManager.Instance.RoomTemplates.PropsData)
                {
                    if (pixelColor == prop.PropColor)
                    {
                        ...
                        propInstance.transform.position = 
                            new Vector3(positions[a].x, positions[a].y, 0f);
    ...
    {
        GameObject doorGO = Instantiate(doorPrefab, objTransform.position,
            Quaternion.identity, objTransform);
