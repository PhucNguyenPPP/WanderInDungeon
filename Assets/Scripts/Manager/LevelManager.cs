...
public class LevelManager : Singleton<LevelManager>
{
    ...
    private bool IsFinalLevelAndDungeon()
    {
        // Check if current level is the last level
        bool isFinalLevel = currentLevelIndex == dungeonLibrary.Levels.Length - 1;

        // Check if current dungeon is the last dungeon in the current level
        bool isFinalDungeon = currentDungeonIndex == dungeonLibrary.Levels[currentLevelIndex].Dungeons.Length - 1;

        // Return true if both are final
        return isFinalLevel && isFinalDungeon;
    }

    private void GameFinished()
    {
        UIManager.Instance.ShowGameFinishedPanel();
    }
